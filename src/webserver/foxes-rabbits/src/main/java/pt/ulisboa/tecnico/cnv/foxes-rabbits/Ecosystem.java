...
public class Ecosystem {
  ...
  private int moveTo(int posI, int posJ, Type destin) {
    ...
    if (posJ < this.N - 1
        && this.aux_board[posI * this.N + (posJ + 1)].getType() == destin) { // try right
      pos[p] = posI * this.N + (posJ + 1);
      p++;
    }
    if (posI < this.M - 1
        && this.aux_board[(posI + 1) * this.N + posJ].getType() == destin) { // try down
      pos[p] = (posI + 1) * this.N + posJ;
      p++;
    }
  ...
  private void rabbitMove(int posI, int posJ, int generation, int parity) {
    ...
    if (new_pos != -1) {
      ...
      // Conflict: rabbit/rabbit
        ...
        // If a rabbit is going to have a child, it competes with breeding_age at 0 and never beats
        // another rabbit
        if (this.rabbit_breeding > this.board[pos].getBreedingAge()
            && this.board[new_pos].getBreedingAge() < this.board[pos].getBreedingAge()) {
  ...
  private void foxMove(int posI, int posJ, int generation, int parity) {
    ...
    if (new_pos != -1) {
      ...
      if (this.board[new_pos].getType() == Type.FOX) {
        ...
        } else if (this.fox_breeding > this.board[pos].getBreedingAge()
            && this.board[new_pos].getBreedingAge() == this.board[pos].getBreedingAge()) {
        ...
        // Case in which the two foxes have a child
            ...
            && this.board[new_pos].getBreedingAge() == 0) {
          if (this.board[new_pos].getStarvingAge() > this.board[pos].getStarvingAge()) {
            this.board[new_pos].setStarvingAge(this.board[pos].getStarvingAge());
          }
        }
    ...
    for (i = 1; i <= n_generations; i++) {
      ...
      for (int j = 0; j < this.M; j++) {
        for (int k = (j + parity) % 2; k < this.N; k += 2) {
          pos = j * this.N + k;
          if (this.aux_board[pos].getType() == Type.EMPTY
              || this.aux_board[pos].getType() == Type.ROCK) {
            continue;
          }
      ...
      // The configuration of the animals is maintained for two consecutive generations and there is
  ...
  public String getCurrentWorldHtmlTable() {
    String world =
        "<style>table, th, td {border:1px solid black;} td { width:40px; height:40px; }</style>";
    ...
    for (int i = 0; i < M; i++) {
      ...
      for (int j = 0; j < N; j++) {
        ...
        switch (this.board[i * N + j].getType()) {
          ...
          case ROCK:
            world +=
                "<img src=\"https://grupos.ist.utl.pt/~meic-cnv.daemon/project/rock.png\""
                    + " width=\"40\" height=\"40\">";
            break;
          case RABBIT:
            world +=
                "<img src=\"https://grupos.ist.utl.pt/~meic-cnv.daemon/project/rabbit.png\""
                    + " width=\"40\" height=\"40\">";
            break;
          case FOX:
            world +=
                "<img src=\"https://grupos.ist.utl.pt/~meic-cnv.daemon/project/fox.png\""
                    + " width=\"40\" height=\"40\">";
            break;
        }
        world += "</td>";
