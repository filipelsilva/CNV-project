...
public class InsectWars {
  ...
  public static final int COLONY_SIZE = 5000; // times two for different insects in each army
  ...
  public static void main(String[] args) {
    ...
    int max = Integer.parseInt(args[0]),
        sz1 = Integer.parseInt(args[1]),
        sz2 = Integer.parseInt(args[2]);
  ...
  public String war(int max, int sz1, int sz2) {
    ...
    while (!army1.isEmpty() && !army2.isEmpty() && ++i < max) {
      ...
      for (j = 0; j < sz1 / 10; j++) {
        ...
        if (i % 2 == 0) {
          // System.out.println(attacker.getName() + " (Army 1) attacks " + defender.getName() + "
          // (Army 2) and deals " + damage + " damage.");
        } else {
          // System.out.println(attacker.getName() + " (Army 2) attacks " + defender.getName() + "
          // (Army 1) and deals " + damage + " damage.");
        }
      }
    ...
    response +=
        "<br>Final iteration: "
            + i
            + " <br>Army1 size: "
            + army1.getInsectCount()
            + " <br>Army2 size: "
            + army2.getInsectCount()
            + "\n";
