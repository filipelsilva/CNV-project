...
public class CompressImageHandlerImpl extends BaseCompressingHandler implements HttpHandler {
  ...
  @Override
  byte[] process(BufferedImage bi, String targetFormat, float compressionQuality)
      throws IOException {
    try {
      ...
      System.out.println(
          String.format(
              "Available compression types for the %s format: %s",
              targetFormat, Arrays.toString(param.getCompressionTypes())));
      System.out.println(param.canWriteCompressed());
      if (compressionType != null) {
        System.out.println(
            String.format(
                "Compression type chosen: %s, compression quality: %f",
  ...
  public static void main(String[] args) throws IOException {
    ...
    byte[] resultImage =
        new CompressImageHandlerImpl().process(bi, targetFormat, compressionQuality);
