...
public class BranchStatistics extends AbstractJavassistTool {
  ...
  public static void printStatistics() {
    for (Map.Entry<String, Map<Integer, Integer>> method : counters.entrySet()) {
      for (Map.Entry<Integer, Integer> basicblock : method.getValue().entrySet()) {
        System.out.println(
            String.format(
                "[%s] %s basic block %s was called %s times",
                BranchStatistics.class.getSimpleName(),
                method.getKey(),
                basicblock.getKey(),
                basicblock.getValue()));
      }
  ...
  protected void transform(CtBehavior behavior) throws Exception {
    ...
    if (behavior.getName().equals("main")) {
      behavior.insertAfter(
          String.format("%s.printStatistics();", BranchStatistics.class.getName()));
    }
  }
  ...
  protected void transform(BasicBlock block) throws CannotCompileException {
    ...
    for (int incoming : block.entrances) {
      System.out.println(
          String.format(
              "[%s] Basic block %s has an outgoing edge to basic block %s",
    ...
    block.behavior.insertAt(
        block.line,
        String.format(
            "%s.visitBasicBlock(\"%s\", %s);",
            BranchStatistics.class.getName(),
            block.getBehavior().getLongName(),
            block.getPosition()));
  }
}
